...
Welcome to 'cherBook, the address book for teachers. This address book application manages and displays student's stats. +
Some feature of this address book includes:

****
* Displays all information of any student for easy reference.
* Statatical graph based on the grades of the list generated.
****

This guide provides information of the software architecture and the API used in this application. This guide is targeted at software developers, be it amateur or professional. +
You can implement your own features or improve on the existing code base to suit your own needs, scope and context.
...
This section describes some noteworthy details on how certain features of the cherBook are implemented. There will be several
diagrams to reinforce understandings of the mechanisms behind the features.
...
"(Student: )(\\d\\d\\d\\d\\d\\d\\d\\d)( Parent: )(\\d\\d\\d\\d\\d\\d\\d\\d)";
----
The regex ensures that the string for `displayed person card phone information` adheres to the
correct format of `Student: (STUDENT_NUMBER) Parent: (PARENT_NUMBER)`, and that the
phone numbers must be exactly 8 digits long.

image::PhoneDisplay.png[width="800"]
_Figure 3.3.1 : Valid Displayed Person Card Phone Information_
...
below the student's name in the person card as seen in the picture above.

image::message_constraint.png[width="1100"]
_Figure 3.3.2 : Message to prompt Users to Enter Correct Phone Format_

If the string format for `displayed person card phone information` is invalid: an IllegalValueException
is thrown. Prompting the user to enter the correct format as seen in the picture above.
[source,java]
----
...
To avoid confusion and messy UI, only one remarks can be stored in any one person at a time

image::PersonCardDifference (Remark).PNG[width="600"]
...
|`* * *` |teacher |sort students by name and classes |find students easily [Coming in v2.0]

|`* * *` |busy teacher |be able to see who are the underperforming students|find out who I should spend more time on [Coming in v2.0]

|`* * *` |teacher |have contact information of students and parents|contact either of them [Coming in v2.0]

|`* * *` |busy teacher |auto tag students according to their grades|get a quick overview of their progress [Coming in v2.0]

|`* * *` |concerned teacher |display statistics for the current group of students in the list|analyse the individual groups' academic progress [Coming in v2.0]

|`* * *` |precise teacher |find out the 25th, 50th and 75th percentile scores for each class|get a better idea of students' academic progress [Coming in v2.0]
...
|`* *` |caring teacher |schedule appointments with my students|conduct extra classes when they ask for it [Coming in v2.0]

|`*` |user |add profile pictures to contacts |find my friends easily [Coming in v2.0]

|`*` |user |set the colour of the tags |can view the tags easily [Coming in v2.0]
...
|`*` |teacher |schedule my timetable according to my classes|plan my timetable to fit other activities [Coming in v2.0]
